...
let WriteReview = ({ product, exit }) => {
  ...
  let [characterCountText, setCharacterCountText] = useState(
    <p>
      review must have at least <em>0</em>/50 charcters
    </p>
  );
  let [summaryCount, setSummaryCount] = useState(
    <p>
      summary must have at most <em>0</em>/60 charcters
    </p>
  );
  ...
  let handleStarClick = (e) => {
    ...
    for (let i = 0; i <= num; i++) {
      temp[i] = (
        <button num={i} key={i} onClick={handleStarClick} className="starButton">
          <BsStarFill />
        </button>
      );
    }
  ...
  let [stars, setStars] = useState([
    <button num="0" key={0} onClick={handleStarClick} className="starButton">
      <BsStar />
    </button>,
    <button num="1" key={1} onClick={handleStarClick} className="starButton">
      <BsStar />
    </button>,
    <button num="2" key={2} onClick={handleStarClick} className="starButton">
      <BsStar />
    </button>,
    <button num="3" key={3} onClick={handleStarClick} className="starButton">
      <BsStar />
    </button>,
    <button num="4" key={4} onClick={handleStarClick} className="starButton">
      <BsStar />
    </button>
  ]);
  ...
  let handleSubmit = (e) => {
    ...
    if (postCheck === true) {
      axios
        .post(prependRequests() + "http://localhost:8080/reviews", body)
        .then((res) => {
          console.log(res);
          exit();
        })
      ...
      for (let i = 0; i < postCheck.length; i++) {
        ...
        setTimeout(() => {
          document.querySelector(postCheck[i]).classList.remove("unacceptable");
        }, 3500);
      }
    }
  };
  ...
  useEffect(() => {
    if (content.length < 50) {
      setCharacterCountText(
        <p>
          review must have at least <em>{content.length}</em>/50 charcters
        </p>
      );
    } else {
      setCharacterCountText(
        <p>
          review must have at most <em>{content.length}</em>/1000 charcters
        </p>
      );
    }
  }, [content]);
  ...
  useEffect(() => {
    if (content.length <= 60) {
      setSummaryCount(
        <p>
          summary must have at most <em>{summary.length}</em>/60 charcters
        </p>
      );
    } else {
      setSummaryCount(
  ...
  return (
    <form className="WriteReview">
      ...
      <div className="writeMidbar">
        <input
          type="text"
          className="writeReviewSummary primaryText"
          id="summary"
          placeholder="A clever summary"
          onChange={(e) => {
      ...
      <textarea
        ...
        id="body"
        placeholder="This item was almost perfect, but ..."
        onChange={(e) => {
